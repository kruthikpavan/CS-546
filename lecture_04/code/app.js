...
const main = async () => {
	...
	try {
		const updatedSashasName = await dogs.updateDog(sasha._id.toString(), 'Dharma', [ 'Husky', 'American Eskimo' ]);
	...
	try {
		const fail1 = await posts.addPost('It was a clear black night', 'a clear white moon', 123);
	} catch (e) {
		console.log(e);
	}

	try {
		const fail1 = await posts.addPost('    ', 'this will fail because of first input', 123);
	} catch (e) {
		console.log(e);
	}
